module Lines
    ...
    def src_lines(pr)
      # chenkx : Remove too long file, such as vendor/d3.v3.js
      count_lines(src_files(pr), lambda{|x| x.length < 5000? true: false})
    end
  
  
    # count the number of lines of test files that the commit related to
    def test_lines(pr)
      count_lines(test_files(pr), lambda{|x| x.length < 5000? true: false})
    end
