module Commits
    ...
    def commit_entries_in_sqlite(pr, at_open = false)
      if at_open
        q = <<-sql
          select c.sha, cd.files , c.parents
          from prs left join pr_commits pc
          on prs.id = pc.pr_id 
          left join commits c 
          on pc.commit_id  = c.id 
          left join commit_details cd 
          on c.id = cd.commit_id 
          where prs.id = ? and prs.created_at >= c.created_at
        sql
        commits = sqlitedb.fetch(q, pr[:id]).all
      end
      commits.select do |c|
    ...
    def commits_on_pr_files(pr, months_back)
      ...
      oldest = Time.at(Time.parse(pr[:created_at]).to_i - 3600 * 24 * 30 * months_back)
      commits = commit_entries_in_sqlite(pr, at_open = true)
      touched_files = []
      commits_per_file = commits.flat_map { |c|
        unless c[:files].nil?
          JSON.parse(c[:files]).map { |f|
            touched_files << f["filename"]
            [c[:sha], f["filename"]]
          }
        else
          []
      ...
      # chenkx:
      # if file number more than 100
      # don't count to reduce run time
      if touched_files.uniq.length > 100
        return {}
      end
    ...
    def files_at_commit(cmt, filter = lambda { |x| true })
      sha = cmt
      begin
    ...
    # from git get repo latest N commits
    def get_repo_latest_commit(cnt)
      tab = []
      walker = Rugged::Walker.new(git)
      walker.sorting(Rugged::SORT_DATE)
      walker.push(git.head.target)
