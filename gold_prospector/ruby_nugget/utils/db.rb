module DB
    ...
    def db
      ...
      Thread.current[:sql_db] ||= Proc.new do
        ...
        Sequel.connect(self.config['sql']['url'], :encoding => 'utf8', :sql_mode => sql_mode) # add the sql_mode to avoid mysql version problem (group by)
      end.call
      Thread.current[:sql_db]
    end

    def sqlitedb
        Thread.current[:sqlite_db] ||= Proc.new do
          Sequel.single_threaded = true
          Sequel.connect("sqlite://#{self.config['sqlite3']['path']}")
        end.call
        Thread.current[:sqlite_db]
    end
